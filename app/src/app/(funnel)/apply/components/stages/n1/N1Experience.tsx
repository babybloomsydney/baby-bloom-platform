...
export function N1Experience({ state, dispatch, goNext, goBack, subPage, progress, questionNumber }: N1ExperienceProps) {
  ...
  const [expTouched, setExpTouched] = useState(experience.total_experience !== null);
  ...
  if (subPage === 'n1-experience') {
    ...
    const totalExp = experience.total_experience ? (isTenPlus ? 10 : parseInt(experience.total_experience) || 0) : 0;
    ...
    const canContinue =
      ...
      (experience.under_3_experience_yn === false || experience.under_3_experience !== null) &&
      (experience.under_3_experience_yn === false ||
        (experience.newborn_experience_yn !== null &&
          (experience.newborn_experience_yn === false || experience.newborn_experience !== null)));
    ...
    return (
      ...
      >
        <div className="flex flex-col gap-5">
            ...
            <div className="p-4 bg-amber-50 border border-amber-200 rounded-lg flex gap-3">
              ...
              <div className="flex flex-col gap-1">
                ...
                <p className="text-sm text-amber-700">
                  Baby Bloom requires all childcare professionals to be at least 18 years of age.
                  Please come back when you&apos;re eligible — we&apos;d love to have you!
                </p>
              </div>
            </div>
          )}
          ...
          <ProgressiveReveal show={experience.date_of_birth !== null && !isUnder18}>
            <div className="flex flex-col gap-2 pt-2">
              ...
              <div className="flex items-center gap-4">
                ...
                <span className="text-sm font-semibold text-violet-600 w-16 text-center">
                  {expSliderValue >= 10 ? '10+' : expSliderValue} {expSliderValue === 1 ? 'year' : 'years'}
                </span>
              </div>
            </div>
          </ProgressiveReveal>
          ...
          {expTouched && totalExp === 0 && (
            <div className="p-4 bg-amber-50 border border-amber-200 rounded-lg flex gap-3">
              ...
              <div className="flex flex-col gap-1">
                ...
                <p className="text-sm text-amber-700">
                  Unfortunately, we require all of our childcare professionals to have at least 1 year of experience working with children. We&apos;d love to welcome you back once you&apos;ve gained a bit more experience!
                </p>
              </div>
            </div>
          )}
          ...
          <ProgressiveReveal show={hasEnoughExp}>
            <div className="flex flex-col gap-2 pt-2">
              <Label className="text-sm font-medium text-slate-700">
                Do you also have experience working with children under 3 years old?
              </Label>
              <YesNoTags
          ...
          <ProgressiveReveal show={experience.under_3_experience_yn === true}>
            <div className="flex flex-col gap-2 pt-2">
              ...
              <div className="flex items-center gap-4">
                ...
                />
                <span className="text-sm font-semibold text-violet-600 w-16 text-center">
                  {formatExp(experience.under_3_experience || 0, under3Max)} {(experience.under_3_experience || 0) === 1 ? 'year' : 'years'}
                </span>
              </div>
            </div>
          </ProgressiveReveal>
          ...
          <ProgressiveReveal show={experience.under_3_experience !== null && experience.under_3_experience > 0 && experience.under_3_experience_yn === true}>
            <div className="flex flex-col gap-2 pt-2">
              <Label className="text-sm font-medium text-slate-700">
            ...
            <div className="flex flex-col gap-2 pt-2">
              ...
              <div className="flex items-center gap-4">
                <input
                  ...
                  onChange={(e) => update({ newborn_experience: parseInt(e.target.value) })}
                  className="flex-1 accent-violet-600"
                />
                <span className="text-sm font-semibold text-violet-600 w-16 text-center">
                  {formatExp(experience.newborn_experience || 0, newbornMax)} {(experience.newborn_experience || 0) === 1 ? 'year' : 'years'}
                </span>
              </div>
            </div>
          </ProgressiveReveal>

          <AutoAdvance shouldAdvance={under3No} onAdvance={goNext} delay={400} />
          <AutoAdvance shouldAdvance={newbornNo && experience.under_3_experience !== null && experience.under_3_experience > 0} onAdvance={goNext} delay={400} />
  ...
  if (subPage === 'n1-roles') {
    ...
    const updateRole = (index: number, field: keyof ChildcareRole, value: string | number) => {
      const updated = [...roles];
    ...
    const canContinue =
      ...
      roles.every((r) =>
        r.role !== '' &&
        r.duration >= 1 &&
        (r.role !== 'Other' || (r.role_other && r.role_other.trim().length > 0))
      );
    ...
    return (
      ...
      >
        <div className="flex flex-col gap-4 pb-24">
          {!noExp && roles.map((role, index) => (
            <div key={index} className="flex flex-col gap-3 p-4 bg-slate-50 rounded-lg border border-slate-200">
              ...
              {role.role === 'Other' && (
                <Input
                  ...
                  onChange={(e) => updateRole(index, 'role_other' as keyof ChildcareRole, e.target.value)}
                  className="h-11 border-slate-200 focus:border-violet-500 focus:ring-violet-500"
                  autoFocus
                />
              )}

              {role.role !== '' && (role.role !== 'Other' || (role.role_other && role.role_other.trim().length > 0)) && (
                <div className="flex flex-col gap-1">
                  ...
                  <div className="flex items-center gap-4">
                    <input
                      type="range"
                      min={0}
                      max={parseInt(experience.total_experience || '10') || 10}
                      value={role.duration}
                      onChange={(e) => updateRole(index, 'duration', parseInt(e.target.value))}
                      className="flex-1 accent-violet-600"
                    />
                    <span className="text-sm font-semibold text-violet-600 w-16 text-center">
                      {experience.total_experience === '10+' && role.duration >= 10 ? '10+' : role.duration} {role.duration === 1 ? 'year' : 'years'}
                    </span>
                  </div>
                </div>
              )}
          ...
          {noExp && (
            <div className="p-4 bg-amber-50 border border-amber-200 rounded-lg flex gap-3">
              ...
              <div className="flex flex-col gap-1">
                ...
                <p className="text-sm text-amber-700">
                  Baby Bloom requires all childcare professionals to have prior experience working with children.
                  If you&apos;re looking to start a career in childcare, we recommend gaining experience through
                  volunteering or formal qualifications first.
                </p>
              </div>
            </div>
          )}
          ...
          {!noExp && (
            <div className="flex items-center justify-between">
              ...
              <button
                type="button"
                onClick={() => update({ no_childcare_experience: true, childcare_roles: [] })}
