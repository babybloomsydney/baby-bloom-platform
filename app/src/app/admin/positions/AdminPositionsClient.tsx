...
import { getConnectionStageBadgeVariant } from "./positions.helpers";
...
function getPositionStageBadgeVariant(
  stage: number | null,
...
function getPositionStatusLabel(pos: AdminPosition): {
  label: string;
  detail: string | null;
} {
  ...
  const label =
    POSITION_STATUS_LABELS[status as PositionStatus] ?? pos.status ?? "Unknown";

  // For active/filled positions, find who filled it
  if (status === POSITION_STATUS.ACTIVE) {
    const hiredConn = pos.connections.find(
      (c) =>
        c.connection_stage === CONNECTION_STAGE.ACTIVE ||
        c.connection_stage === CONNECTION_STAGE.CONFIRMED,
    );
...
function getPositionStatusBadgeVariant(
  status: number | null,
): "active" | "pending" | "verified" | "inactive" | "failed" | "info" {
...
function matchesStageFilter(pos: AdminPosition, filter: StageFilter): boolean {
  ...
  switch (filter) {
    case "active":
      return stage === POSITION_STAGE.OPEN;
    case "connecting":
      return stage === POSITION_STAGE.CONNECTING;
    case "filled":
      return stage === POSITION_STAGE.ACTIVE;
    case "closed":
      return stage === POSITION_STAGE.ENDED || stage === POSITION_STAGE.CLOSED;
    default:
      return true;
  }
}
...
export function AdminPositionsClient({ positions }: Props) {
  ...
  const activeCount = positions.filter(
    (p) => p.stage === POSITION_STAGE.OPEN,
  ).length;
  const connectingCount = positions.filter(
    (p) => p.stage === POSITION_STAGE.CONNECTING,
  ).length;
  const filledCount = positions.filter(
    (p) => p.stage === POSITION_STAGE.ACTIVE,
  ).length;
  const closedCount = positions.filter(
    (p) =>
      p.stage === POSITION_STAGE.ENDED || p.stage === POSITION_STAGE.CLOSED,
  ).length;
            ...
            <Table>
              ...
              <TableBody>
                {filtered.map((pos) => {
                  ...
                  const displayName =
                    pos.family_display_name ?? pos.parent_name ?? "Unknown";
                  return (
                    <>
                      <TableRow
                        key={pos.id}
                      ...
                      >
                          ...
                          {(() => {
                            ...
                            return (
                              <div className="flex flex-col gap-0.5">
                                <StatusBadge
                                  variant={getPositionStatusBadgeVariant(
                                    pos.position_status,
                                  )}
                                >
                                  {label}
                        ...
                        <TableCell>
                          <StatusBadge
                            variant={getSourceBadgeVariant(pos.source)}
                          >
...
function PositionDetail({
  ...
  const activeConnections = position.connections.filter(
    (c) =>
  ...
  const closedConnections = position.connections.filter(
    (c) => !activeConnections.includes(c),
  );
  ...
  const isClosed =
    position.stage === POSITION_STAGE.ENDED ||
    position.stage === POSITION_STAGE.CLOSED;
    ...
    <div className="space-y-4 px-6 py-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3 text-sm">
          ...
          {position.source === "parent" && position.parent_name && (
            <button
              onClick={() =>
                position.parent_user_id && onOpenUser(position.parent_user_id)
              }
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-2 text-sm text-slate-600">
        {position.hourly_rate != null && (
          <span>
            <strong>Rate:</strong> ${position.hourly_rate}/hr
          </span>
        )}
        {position.hours_per_week != null && (
          <span>
            <strong>Hours:</strong> {position.hours_per_week}h/wk
          </span>
        )}
        {position.schedule_type && (
          <span>
            <strong>Schedule:</strong> {position.schedule_type}
          </span>
        )}
        {position.days_required && (
          <span>
            <strong>Days:</strong> {position.days_required.join(", ")}
          </span>
        )}
        {position.placement_length && (
          <span>
            <strong>Length:</strong> {position.placement_length}
          </span>
        )}
        {position.dfy_activated_at && (
          <span>
            <strong>DFY:</strong> {position.dfy_tier ?? "standard"} — activated{" "}
            {formatDate(position.dfy_activated_at)}
          </span>
        )}
      </div>
            ...
            <div className="flex gap-1">
              {position.children.map((c, i) => (
                ...
                >
                  ...
                  {formatGender(c.gender) ? ` ${formatGender(c.gender)}` : ""}
                </span>
              ))}
            </div>
          </div>
      ...
      {position.description && (
        <p className="text-sm text-slate-500 italic">
          &ldquo;{position.description}&rdquo;
        </p>
      )}
      ...
      {activeConnections.length > 0 && (
        <div>
          <h4 className="mb-2 text-sm font-semibold text-slate-700">
            Active Applicants ({activeConnections.length})
          </h4>
      ...
      {closedConnections.length > 0 && (
        <div>
          <h4 className="mb-2 text-sm font-semibold text-slate-500">
            Closed / Terminal ({closedConnections.length})
          </h4>
...
function ConnectionTable({
  ...
  connections: AdminPosition["connections"];
}) {
  return (
        ...
        <tbody className="divide-y divide-slate-100">
          {connections.map((conn) => (
            <tr key={conn.id} className="hover:bg-white">
              ...
              <td className="px-3 py-2">
                <StatusBadge
                  variant={getConnectionStageBadgeVariant(
                    conn.connection_stage,
                  )}
                >
                  {conn.connection_stage != null
                    ? (CONNECTION_STAGE_LABELS[
                        conn.connection_stage as ConnectionStage
                      ] ?? `Stage ${conn.connection_stage}`)
                    : "Unknown"}
                </StatusBadge>
              </td>
              ...
              <td className="px-3 py-2 text-slate-500">
                {conn.responded_at ? timeAgo(conn.responded_at) : "—"}
              </td>
              <td className="px-3 py-2 text-slate-500">
                {formatDate(conn.confirmed_time)}
              </td>
            </tr>
          ))}
        </tbody>
        ...
        <thead className="bg-slate-100">
          <tr>
            ...
            {SCHEDULE_BRACKETS.map((b) => (
              <th
                key={b}
                className="px-2 py-1.5 text-center font-medium text-slate-600 capitalize"
              >
                {b}
              </th>
            ))}
