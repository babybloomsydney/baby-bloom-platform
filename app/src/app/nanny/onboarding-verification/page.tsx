...
export default async function OnboardingVerificationPage() {
  ...
  const [verificationResult, profileResult, nannyResult] = await Promise.all([
    ...
    admin
      .from('user_profiles')
      .select('first_name, last_name, date_of_birth, mobile_number, suburb, postcode, profile_picture_url')
      .eq('user_id', user.id)
      .single(),
    admin
      .from('nannies')
  ...
  const rawHeadline = typeof aiContent?.headline === 'string' ? aiContent.headline : null;
  ...
  if (verification) {
    ...
    if (verification.contact_status === 'saved' && verification.identity_status === 'not_started') {
    ...
    if (verification.identity_status === 'failed' || verification.identity_status === 'rejected') {
    ...
    if (verification.identity_status !== 'not_started' && verification.contact_status !== 'saved') {
    ...
    const allDone =
      ...
      (verification.wwcc_status === 'doc_verified' || verification.wwcc_status === 'review') &&
