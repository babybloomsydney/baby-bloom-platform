-- ============================================================================
-- Apply WU 9.1 — extend the `logging_rules` katie_prompt section to teach
-- Katie about the [Image attached: <url>] marker.
--
-- Pre-WU-9.1: KATIE_IMAGE_MARKER_ENABLED defaulted to false because the
-- prompt didn't mention the marker. With this update, the prompt teaches
-- Katie to recognise the marker and route to the appropriate propose
-- tool — and the flag default flips to true.
--
-- Same atomic deactivate-then-insert pattern as previous apply scripts
-- (apply-voice-tone-fix, apply-focus-rule, apply-draft-flow). The
